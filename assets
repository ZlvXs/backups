...
--[[if withoutware.Options["FPS Unlocker"].Enabled then 
    setfpscap(withoutware.Options["FPS Unlocker"].FPS)
end]]

for i,v in pairs(game.Workspace:GetDescendants()) do 
    if v:IsA("Seat") then 
        v.Disabled = withoutware["Extra"]["Disable Seats"]
    end
end
...
if not withoutware["Extra"]["Low Graphics"] then
...
if table.find(Blacklisted_GameIds,game.PlaceId) then 
    game:GetService("StarterGui"):SetCore("SendNotification",{
        ...
        Text = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.." is blacklisted because it is unsafe to use withoutware on.", 
...
if withoutware.Options.Version ~= utility.Version then 
    game:GetService("StarterGui"):SetCore("SendNotification",{
...
task.spawn(function()
    pcall(function()
        ...
        Memory:GetPropertyChangedSignal("Text"):Connect(function()
            if withoutware.Spoofer.Memory.Enabled then 
                local random1 = math.random(withoutware.Spoofer.Memory.Min, withoutware.Spoofer.Memory.Max)
...
game:GetService("UserInputService").InputBegan:Connect(function(key, gp)
    if not gp then 
        if key.KeyCode == withoutware.Emotes.LayKey then 
            if withoutware.Emotes.Lay then 
        ...
        if key.KeyCode == withoutware.Emotes.GreetKey then
            if withoutware.Emotes.Greet then 
        ...
        if key.KeyCode == withoutware.Emotes.SpeedKey then
            if withoutware.Emotes.Speed then 
        ...
        if key.KeyCode == withoutware.Emotes.SturdyKey then
            if withoutware.Emotes.Sturdy then 
        ...
        if key.KeyCode == withoutware.Emotes.GriddyKey then
            if withoutware.Emotes.Griddy then 
...
local notifications = notificationLibrary.new({            
    NotificationLifetime = withoutware.Options["Notifcation Duration"], 
...
getgenv().notify = function(txt) 
    if withoutware.Options["Allow Notifications"] then
        if string.lower(withoutware.Options["Notifcation Type"]) == "akali" then 
            gggg({
                Description = txt;
                Title = "Notification";
                Duration = withoutware.Options["Notifcation Duration"];
            });
        elseif string.lower(withoutware.Options["Notifcation Type"]) == "roblox" then
            game:GetService("StarterGui"):SetCore("SendNotification",{
                Title = "Notification",
                Text = txt, 
        ...
        elseif string.lower(withoutware.Options["Notifcation Type"]) == "xaxa" then
...
local closestsilentbodypart = withoutware["Silent Aim"]["Target Part"]
local closestcamlockpart = withoutware["Cam Lock"]["Target Part"]
...
if withoutware["Options"]["Intro"] then 
...
Circle.Transparency = withoutware.FOV["Silent Aim"].Transparency
Circle.Radius = withoutware.FOV["Silent Aim"].Size * 3
Circle.Visible = withoutware.FOV["Silent Aim"].Visible
Circle.Color = withoutware.FOV["Silent Aim"].Color
Circle.Thickness = withoutware.FOV["Silent Aim"].Thickness
Circle.Filled = withoutware.FOV["Silent Aim"].Filled

local Circle2 = Drawing.new("Circle")
Circle2.Transparency = withoutware.FOV["Cam Lock"].Transparency
Circle2.Radius = withoutware.FOV["Cam Lock"].Size * 3
Circle2.Visible = withoutware.FOV["Cam Lock"].Visible
Circle2.Color = withoutware.FOV["Cam Lock"].Color
Circle2.Thickness = withoutware.FOV["Cam Lock"].Thickness
Circle2.Filled = withoutware.FOV["Cam Lock"].Filled
...
UpdateESP = function()
    for i,v in pairs(EspPlayers) do
        if withoutware.ESP.Enabled and i ~= LocalPlayer and i.Character and i.Character:FindFirstChild("Humanoid") and i.Character:FindFirstChild("HumanoidRootPart") and i.Character:FindFirstChild("Head") then
            ...
            if OnScreen and GetDistance(i) < withoutware.ESP["Max Distance"] then
                if withoutware.ESP.Name.Enabled then
                    v.Name.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BoxPos.Y - 16)
                    v.Name.Outline = withoutware.ESP.Name.Outline
                    v.Name.Text = i.DisplayName.." (@"..i.Name..")"
                    v.Name.Color = withoutware.ESP.Name.Color
                ...
                if withoutware.ESP.Distance.Enabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    v.Distance.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BottomOffset)
                    v.Distance.Outline = withoutware.ESP.Distance.Outline
                    v.Distance.Text = math.floor((Hrp.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude) .. " studs"
                    v.Distance.Color = withoutware.ESP.Distance.Color
                ...
                if withoutware.ESP.Box.Enabled then
                    v.BoxOutline.Size = BoxSize
                    v.BoxOutline.Position = BoxPos
                    v.BoxOutline.Visible = withoutware.ESP.Box.Outline
                    v.BoxOutline.Color = Color3.fromRGB(0, 0, 0)
    
                    v.Box.Size = BoxSize
                    v.Box.Position = BoxPos
                    v.Box.Color = withoutware.ESP.Box.Color
                    v.Box.Visible = true
                else
                    v.BoxOutline.Visible = false
                    v.Box.Visible = false
                end
                if withoutware.ESP["Health Bar"].Enabled then
                    ...
                    v.HealthBar.Color = withoutware.ESP["Health Bar"].Color
                    ...
                    v.HealthBarOutline.Visible = withoutware.ESP["Health Bar"].Outline
                else
...
GetPrediction = function() 
    if withoutware["Global"]["Auto Prediction"].Enabled then 

        local PingStats = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
        local Value = tostring(PingStats)
        local PingValue = Value:split(" ")
        local PingNumber = tonumber(PingValue[1])
        if string.lower(withoutware.Global["Auto Prediction"]["Version"]) == "default" then 
            return tonumber(PingNumber / 225 * 0.1 + 0.1)
        elseif string.lower(withoutware.Global["Auto Prediction"]["Version"]) == "old" then 
            if PingNumber < 130 then
                return tonumber(PingNumber / 1000 + 0.037)
            else
                return tonumber(PingNumber / 1000 + 0.033)
            end
        elseif string.lower(withoutware.Global["Auto Prediction"]["Version"]) == "v2" then 
        ...
        elseif string.lower(withoutware.Global["Auto Prediction"]["Version"]) == "custom" then 
            if PingNumber < 30 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping20)
            elseif PingNumber < 40 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping30)
            elseif PingNumber < 50 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping40)
            elseif PingNumber < 60 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping50)
            elseif PingNumber < 70 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping60)
            elseif PingNumber < 80 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping70)
            elseif PingNumber < 90 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping80)
            elseif PingNumber < 100 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping90)
            elseif PingNumber < 110 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping100)
            elseif PingNumber < 120 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping110)
            elseif PingNumber < 130 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping120)
            elseif PingNumber < 140 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping130)
            elseif PingNumber < 150 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping140)
            elseif PingNumber < 160 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping150)
            elseif PingNumber < 170 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping160)
            elseif PingNumber < 180 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].P1ing70)
            elseif PingNumber < 190 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping180)
            elseif PingNumber < 200 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping190)
            elseif PingNumber < 210 then
                return tonumber(withoutware.Global["Auto Prediction"]["Custom Settings"].Ping200)
            end
        end
    else
        return tonumber(withoutware["Silent Aim"].Prediction)
    end
end
...
CheckDistance = function(Player)
    if (Player.Character.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > withoutware.Checks.Distance["Max Distance"] then 
        return true
    else
        return false
    end
...
GetClosestBodyPart = function()
    ...
    if withoutware["Silent Aim"]["Nearest Part To Cursor"] then 
        closestsilentbodypart = tostring(BodyPart)
    else
        closestsilentbodypart = withoutware["Silent Aim"]["Target Part"]
    end
...
GetClosestBodyPart2 = function()
    ...
    if withoutware["Cam Lock"]["Nearest Part To Cursor"] then 
        closestcamlockpart = tostring(BodyPart)
    else
        closestcamlockpart = withoutware["Cam Lock"]["Target Part"] 
    end
end
...
GetClosestPlayer = function()
    ...
    local HitChance = CalculateChance(withoutware["Silent Aim"]["Hit Chance"])
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v.Character and v ~= LocalPlayer and v.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if withoutware.Checks["Visible Check"] and not RayCastCheck(v.Character.HumanoidRootPart, v.Character) then 
                continue 
            end
            if withoutware.Checks["Whitelist"].Enabled and not table.find(withoutware.Checks.Whitelist.Usernames,v.Name) then 
                continue 
            end
            if withoutware.Checks["Blacklist"].Enabled and table.find(withoutware.Checks.Blacklist.Usernames,v.Name) then 
                continue 
            end
            if table.find(safeplayers,v.UserId) then 
                continue 
            end
            if withoutware.Checks["K.O Check"] and v.Character:FindFirstChild("BodyEffects") then
                local KoCheck 
            ...
            if withoutware.Checks["Crew Check"] and FindCrew(v) and v.DataFolder.Information:FindFirstChild("Crew").Value == LocalPlayer.DataFolder.Information:FindFirstChild("Crew").Value then
                continue
            end
            if withoutware.Checks["Friend Check"] and game.Players.LocalPlayer:IsFriendsWith(v.UserId) then
                continue
            end

            if withoutware.Checks["Distance"].Enabled and CheckDistance(v) then
                continue
            end
...
game:GetService("RunService").RenderStepped:Connect(function()
    if withoutware["Silent Aim"].Enabled and withoutware["Silent Aim"]["Closest Point"] and SilentTarget ~= nil and SilentTarget.Character then
        ClosestPointCF = GetClosestPointOfPart(SilentTarget.Character[closestsilentbodypart])
    end
end)

game:GetService("RunService").RenderStepped:Connect(function()
    if withoutware["Cam Lock"].Enabled and withoutware["Cam Lock"]["Closest Point"] and CamlockTarget ~= nil and CamlockTarget.Character then
...
function SortInventory() 
     ...
     for x=1,#withoutware["Gun Sorting"].Slots do
         for i=1,#temp do
             if string.sub(string.lower(tostring(temp[i])),1,string.len(string.lower(withoutware["Gun Sorting"].Slots[x]))) == string.lower(withoutware["Gun Sorting"].Slots[x]) then
                 temp[i].Parent = variables.LocalPlayer.Backpack
                 wait(0.105)
             end
         end
     end
     if withoutware["Gun Sorting"]["Delete Extra"] then 
        for i=1,#temp do
            if not table.find(withoutware["Gun Sorting"].Slots,temp[i].Name) then 
                temp[i]:Destroy()
                wait(0)
            end
        end
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = withoutware["Gun Sorting"].Bind:lower()
    if (Key == Keybind) then
        if withoutware["Gun Sorting"].Enabled then 
            SortInventory()
        end
    end
end)
...
RunService.Heartbeat:Connect(function()
    UpdateFOV()
    if withoutware["Silent Aim"].Enabled then 
        if string.lower(withoutware["Silent Aim"].Mode) == "fov" then 
            GetClosestPlayer()
        end
    ...
    if withoutware["Cam Lock"].Enabled then 
        if string.lower(withoutware["Cam Lock"].Mode) == "fov" then 
            GetClosestPlayer2()
        end
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = withoutware["Silent Aim"]["Target Mode Bind"]:lower()
    local Keybind2 = withoutware["Cam Lock"]["Target Mode Bind"]:lower()
    if (Key == Keybind) and withoutware["Silent Aim"].Enabled then
        if string.lower(withoutware["Silent Aim"].Mode) ==  "target" then 
            if SilentTarget == nil then 
                GetClosestPlayer()
                if withoutware["Silent Aim"].Notify then 
                    if SilentTarget ~= nil then 
    ...
    if (Key == Keybind2 and withoutware["Cam Lock"].Enabled) then
        if string.lower(withoutware["Cam Lock"].Mode) ==  "target_toggle" then 
            if CamlockTarget == nil then 
                GetClosestPlayer2()
                if withoutware["Cam Lock"].Notify then 
                    if CamlockTarget ~= nil then 
...
mouse.KeyDown:Connect(function(Key)
    local Keybind2 = withoutware["Cam Lock"]["Target Mode Bind"]:lower()
    if (Key == Keybind2 and withoutware["Cam Lock"].Enabled) then
        if string.lower(withoutware["Cam Lock"].Mode) ==  "target_hold" then 
            if CamlockTarget == nil then 
                GetClosestPlayer2()
                if withoutware["Cam Lock"].Notify then 
                    if CamlockTarget ~= nil then 
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(withoutware.Macro.Bind)) and withoutware.Macro.Abuse == false then
        if withoutware.Macro.Enabled then
            speeding = not speeding
            if speeding == true then
                repeat task.wait(withoutware.Macro.Speed / 100)
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "I", false, game)
                task.wait(withoutware.Macro.Speed / 100)
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(withoutware.Macro.Bind)) and withoutware.Macro.Abuse == true then
        if withoutware.Macro.Enabled  then
            speeding = not speeding
            if speeding == true then
                repeat task.wait(withoutware.Macro.Speed / 100)
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(withoutware["Fake Spike"].Bind))then
        if withoutware["Fake Spike"].Enabled then
            settings():GetService("NetworkSettings").IncomingReplicationLag = 99999
            wait(withoutware["Fake Spike"].Duration)
...
spawn(function()
    while wait(0.5) do 
        if withoutware["Extra"]["Mute Boomboxes"] == true then 
...
mouse.KeyDown:Connect(function(Key)
    if withoutware["Panic"].Enabled then
        if Key == (string.lower(withoutware["Panic"].Bind))then
            if withoutware["Panic"]["Things To Disable"].Options then
                withoutware["Options"].Stats = false
                withoutware["Options"]["Allow Notifications"] = false
            end

            if withoutware["Panic"]["Things To Disable"].Aiming then
                withoutware["Silent Aim"].Enabled = false
                withoutware["Cam Lock"].Enabled = false
            end

            if withoutware["Panic"]["Things To Disable"].FOV then
                withoutware.FOV["Silent Aim"].Visible = false
                withoutware.FOV["Cam Lock"].Visible = false
            end

            if withoutware["Panic"]["Things To Disable"].Misc then
                withoutware["Macro"].Enabled = false
                withoutware["Gun Sorting"].Enabled = false
                withoutware["Fake Spike"].Enabled = false
                withoutware["Chat Commands"].Enabled = false
                withoutware["Hit Sound"].Enabled = false
                withoutware["Hit Log"].Enabled = false
            end

            if withoutware["Panic"]["Things To Disable"].Emotes then
                withoutware["Emotes"].Lay = false
                withoutware["Emotes"].Greet = false
                withoutware["Emotes"].Speed = false
                withoutware["Emotes"].Sturdy = false
                withoutware["Emotes"].Griddy = false
            end
        end
    end
end)
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = withoutware.Resolver.Keybind:lower()
    if (Key == Keybind) then
        if withoutware.Resolver["Keybind Enabled"] then 
            if withoutware.Resolver.Enabled == true then 
                withoutware.Resolver.Enabled = false
                notify("Resolver disabled.")
            else
                withoutware.Resolver.Enabled = true
...
RunService.Heartbeat:Connect(function()
    if withoutware.Resolver.Enabled and withoutware["Silent Aim"].Enabled then 
        if withoutware.Resolver.Method == "Delta" then 
            if SilentTarget ~= nil then 
    ...
    if SilentTarget ~= nil then 
        ...
        if withoutware["Silent Aim"]["Closest Point"] then
        ...
        TargetCF.Position + v3.new(0,withoutware["Silent Aim"]["Target Part Offset"],0) + 
            (SilentTarget.Character[closestsilentbodypart].AssemblyLinearVelocity *
            withoutware["Silent Aim"].Prediction))


    end
    if withoutware["Silent Aim"].Dot and SilentTarget ~= nil and pos then
    ...
    if withoutware["Silent Aim"].Line and SilentTarget ~= nil and pos then
...
RunService.Heartbeat:Connect(function()
    if withoutware.Resolver.Enabled and withoutware["Cam Lock"].Enabled then 
        if withoutware.Resolver.Method == "Delta" then 
            if CamlockTarget ~= nil then 
    ...
    if CamlockTarget ~= nil then 
        ...
        if withoutware["Cam Lock"]["Closest Point"] then
        ...
        pos = workspace.CurrentCamera:WorldToViewportPoint(
        TargetCF.Position + v3.new(0,withoutware["Cam Lock"]["Target Part Offset"],0) + 
            (CamlockTarget.Character[closestcamlockpart].AssemblyLinearVelocity *
            withoutware["Cam Lock"].Prediction))


    end
    if withoutware["Cam Lock"].Dot and CamlockTarget ~= nil and pos then
    ...
    if withoutware["Cam Lock"].Line and CamlockTarget ~= nil and pos then
...
--[[LPH_NO_VIRTUALIZE(function()
    ...
    oldIndex = hookmetamethod(game, "__index", function(self, Index, Check)
        ...
        if self == Mouseee and (Index:lower() == OldHit) then    
            if withoutware["Silent Aim"].Enabled and withoutware["Resolver"]["Aim Viewer Bypass"] == false then 
                if SilentTarget ~= nil then
                    ...
                    if withoutware["Silent Aim"].Shake.Enabled then 
                        shake = v3.new(
                            math.random(-withoutware["Silent Aim"].Shake.X, withoutware["Silent Aim"].Shake.X),
                            math.random(-withoutware["Silent Aim"].Shake.Y, withoutware["Silent Aim"].Shake.Y),
                            math.random(-withoutware["Silent Aim"].Shake.Z, withoutware["Silent Aim"].Shake.Z)
                        ) * 0.1
                    end

                    if withoutware.Airshot.Enabled then 
                        if SilentTarget.Character.Humanoid.FloorMaterial == Enum.Material.Air then 
                            offset = v3.new(0,withoutware.Airshot["Jump Offset"],0)
                        end
                    end
                    ...
                    if withoutware.Resolver.Enabled then 
                        if withoutware.Resolver["Auto Detect"] then 
                    ...
                    if resolver then 
                        if withoutware.Resolver.Method == "Move Direction" then 
                        ...
                        elseif withoutware.Resolver.Method == "No Prediction" then 
                            targetbone.Velocity = v3.new(0,0,0)
                            targetbone.AssemblyLinearVelocity = v3.new(0,0,0)
                        elseif withoutware.Resolver.Method == "Delta" or  withoutware.Resolver.Method == "Recalculate" then 
                            targetbone.Velocity = newvel
                            targetbone.AssemblyLinearVelocity = newvel
                        end
                    end
                    if withoutware["Silent Aim"]["Anti Ground Shots"] then 
                    ...
                    local Prediction = 
                    TargetCF + 
                    v3.new(0,withoutware["Silent Aim"]["Target Part Offset"],0) + 
                    offset + 
                    targetbone.Velocity * 
        ...
        tool.Activated:Connect(function()
            if withoutware["Silent Aim"].Enabled then 
                if SilentTarget ~= nil then
                    ...
                    if withoutware["Global"]["Anti Ground Shots"] then 
...
LPH_NO_VIRTUALIZE(function()
    game:GetService("RunService").RenderStepped:Connect(function()
        if withoutware["Global"]["Unlock Outside FOV"] and CamlockTarget and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
            if Circle2.Radius <
                (Vector2.new(
        ...
        if withoutware["Global"]["Unlock On KO"] == true and CamlockTarget ~= nil and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
        ...
        if withoutware["Global"]["Unlock Behind Wall"] == true and string.lower(withoutware["Cam Lock"].Mode) ~= "fov" and CamlockTarget ~= nil and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
        ...
        if withoutware["Cam Lock"].Enabled and CamlockTarget and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then 
            if CamlockTarget ~= nil then
                ...
                if withoutware["Cam Lock"].Shake.Enabled then 
                    shake = v3.new(
                        math.random(-withoutware["Cam Lock"].Shake.X, withoutware["Cam Lock"].Shake.X),
                        math.random(-withoutware["Cam Lock"].Shake.Y, withoutware["Cam Lock"].Shake.Y),
                        math.random(-withoutware["Cam Lock"].Shake.Z, withoutware["Cam Lock"].Shake.Z)
                    ) * 0.1
                end

                if withoutware["Cam Lock"].Smoothness.Enabled then 
                    smoothing = withoutware["Cam Lock"].Smoothness.Value
                end

                if withoutware.Airshot.Enabled then 
                    if CamlockTarget.Character.Humanoid.FloorMaterial == Enum.Material.Air then 
                        offset = v3.new(0,withoutware.Airshot["Jump Offset"],0)
                        if withoutware.Airshot["Cam Lock Smoothness"].Enabled then 
                            smoothing = withoutware.Airshot["Cam Lock Smoothness"]["New Smoothness"]
                        end
                    end
                end
                ...
                if resolver then 
                    ...
                    elseif withoutware.Resolver.Method == "No Prediction" then 
                        targetbone.Velocity = v3.new(0,0,0)
                        targetbone.AssemblyLinearVelocity = v3.new(0,0,0)
                    elseif withoutware.Resolver.Method == "Delta" then 
                        targetbone.Velocity = newvel
                        targetbone.AssemblyLinearVelocity = newvel
                    end
                end
                ...
                local Prediction = 
                TargetCF + 
                v3.new(0,withoutware["Cam Lock"]["Target Part Offset"],0) + 
                offset + 
                targetbone.Velocity * 
                PredictionValue2 + shake
                local Main = CF.new(CurrentCamera.CFrame.p, Prediction)
                CurrentCamera.CFrame = CurrentCamera.CFrame:Lerp(Main, smoothing, withoutware["Cam Lock"]["Easing Style"] , Enum.EasingDirection.InOut)
            end
        end
    end)
end)()
        ...
        connection = char:WaitForChild("Humanoid").HealthChanged:Connect(function(health)
            if health < oldhealth then 
                if plr == SilentTarget or plr == CamlockTarget then 
                    if withoutware["Hit Sound"].Enabled then 
                        local Sound = Instance.new('Sound', CurrentCamera)
                        local SoundName = withoutware["Hit Sound"]["Hit Sound"]
                        local SoundID = hitSounds[SoundName]
                        Sound.SoundId = SoundID
                        Sound.Volume = withoutware["Hit Sound"].Volume
                        Sound:Play()
                        delay(10,function()
                            Sound:Destroy()
                        end)
                    end
                    if withoutware["Hit Logs"].Enabled then 
...
game.Players.LocalPlayer.Chatted:Connect(function(msg)
    ...
    if withoutware["Chat Commands"].Enabled then 
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.Prediction) then 
            if tonumber(text[2]) then 
                withoutware["Silent Aim"].Prediction = tonumber(text[2])
            else
                invalidargs(text[2])
            end
        end
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.ToggleFOV) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                withoutware.FOV["Silent Aim"].Visible = true
        ...
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.SizeFOV) then 
            if tonumber(text[2]) then 
                withoutware.FOV["Silent Aim"].Size = tonumber(text[2]) * 3
                withoutware.FOV["Silent Aim"].Size = tonumber(text[2]) * 3
            else
                invalidargs(text[2])
            end
        end
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.GotoLocation) then 
            if game.PlaceId == 2788229376 or game.PlaceId == 7213786345 then
        ...
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.ToggleResolver) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                withoutware.Resolver.DesyncResolver = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                withoutware.Resolver.DesyncResolver = false
            else
                invalidargs(text[2])
            end
        end

        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.JoinDiscord) then 
            if httprequest then
                httprequest({
                    Url = 'http://127.0.0.1:6463/rpc?v=1',
                    Method = 'POST',
        ...
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.RejoinServer) then 
            game:GetService("TeleportService"):Teleport(game.PlaceId, game.JobId, game:GetService("Players").LocalPlayer)
        end

        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.ToggleSilentAim) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                withoutware["Silent Aim"].Enabled = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                withoutware["Silent Aim"].Enabled = false
            else
                invalidargs(text[2])
        ...
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.ToggleCamLock) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                withoutware["Silent Aim"].Enabled = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                withoutware["Silent Aim"].Enabled = false
        ...
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.CamLockSmoothness) then 
            if tonumber(text[2]) then 
                withoutware["Silent Aim"].Smoothness = tonumber(text[2])
            else
                invalidargs(text[2])
            end
        end

        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.CheckForExploiters) then 
            local found = false
            for i,v in pairs(game.Players:GetChildren()) do
        ...
        if string.lower(text[1]) == string.lower(withoutware["Chat Commands"].Prefix..withoutware["Chat Commands"].Commands.ForceReset) then 
            if game.PlaceId == 2788229376 or game.PlaceId == 7213786345 then
                ...
                game.Players.LocalPlayer.Character.Name = 'withoutware ; #1'
            else
                unsupportedgame()
            end
        end
...
Mark.Text = "withoutware v"..utility.Version.." | .gg/"..utility.Invite
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    if withoutware.Options.Stats then 
        Mark.Visible = true
        Pred.Visible = true
        Pred.Text = "Prediction: "..tostring(PredictionValue)
        Resolver.Visible = true
        Resolver.Text = "Resolver: "..tostring(withoutware.Resolver.Enabled).." | "..withoutware.Resolver.Method.." | {"..withoutware.Resolver.Keybind.."}"
        Ping22.Visible = true
        Ping22.Text = "Ping: "..tostring(utility.Ping).."ms"
        silenttarg.Visible = true
        silenttarg.Text = "Silent Aim Target: "..tostring(SilentTarget).." | "..closestsilentbodypart.." | "..GetHealth(SilentTarget).." | "..GetDistance(SilentTarget).." studs | {"..withoutware["Silent Aim"]["Target Mode Bind"].."}"
        camlocktarg.Visible = true
        camlocktarg.Text = "Camlock Target: "..tostring(CamlockTarget).." | "..closestcamlockpart.." | "..GetHealth(CamlockTarget).." | "..GetDistance(CamlockTarget).." studs | {"..withoutware["Cam Lock"]["Target Mode Bind"].."}"
    else
        Mark.Visible = false
        Pred.Visible = false
